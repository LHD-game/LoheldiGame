Shader "Hidden/NHP/UnderwaterFog"
{
	Properties {
	...
	CGINCLUDE
		...
		half4 ComputeFog(v2f i, bool distance, bool height) : SV_Target
		{
			...
			float2 uvDisp = tex2D(_DistortionTex, i.uv + _Time.y * _DistortionSpeed).xy;
			uvDisp = ((uvDisp * 2) - 1) * _Distortion * 0.1;
			...
			half4 underwaterColor = lerp(sceneColor, fogColor, saturate(fogMask * _Color.a));
